...
class PerfProfiler(BaseProfiler):
    ...
    def _generate_reports(self):
        ...
        with open(self.dso_report, "w") as f:
            ...
            if result.returncode != 0:
                print(
                    f"WARNING: perf report (dso) failed: {result.stderr.decode().strip()}"
                )
        ...
        with open(self.symbols_report, "w") as f:
            ...
            if result.returncode != 0:
                print(
                    f"WARNING: perf report (symbols) failed: {result.stderr.decode().strip()}"
                )
        ...
        with open(self.callgraph_report, "w") as f:
            ...
            if result.returncode != 0:
                print(
                    f"WARNING: perf report (callgraph) failed: {result.stderr.decode().strip()}"
                )
    ...
    def generate_flamegraph(self, flamegraph_dir: Path) -> Optional[Path]:
        ...
        if not stackcollapse.exists() or not flamegraph_pl.exists():
            print(
                f"WARNING: FlameGraph tools not found in {flamegraph_dir}",
                file=sys.stderr,
            )
            return None
