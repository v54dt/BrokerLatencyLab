...
class StraceProfiler(BaseProfiler):
    ...
    def parse_output(self) -> ProfilerResult:
        ...
        if not events:
            print("WARNING: No syscall events found in strace log", file=sys.stderr)
        ...
        with open(self.log_file) as f:
            for line in f:
                ...
                if match:
                    ...
                    # Extract syscall name (handle resumed format)
                    if "<... " in line and " resumed>" in line:
                        resumed_match = re.search(r"<\.\.\.\s+(\w+)\s+resumed>", line)
                        syscall_name = (
                            resumed_match.group(1)
                            if resumed_match
                            else syscall.split("(")[0]
                        )
                    else:
                        syscall_name = (
                            syscall.split("(")[0] if "(" in syscall else syscall
                        )
    ...
    def _analyze_events(self, events: List[SyscallEvent]) -> Dict[str, Any]:
        ...
        Uses interval merging for multi-threaded programs where
        epoll_wait calls may overlap across threads.
        """
        ...
        # Merge overlapping wait intervals for accurate network wait time
        if not waits:
            network_wait_ms = 0.0
        else:
            intervals = [(e.timestamp, e.timestamp + e.duration) for e in waits]
            intervals.sort()
            ...
            network_wait_ms = sum((end - start) * 1000 for start, end in merged)
