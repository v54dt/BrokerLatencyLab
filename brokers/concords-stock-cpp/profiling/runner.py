"""Profiling runner that orchestrates a profiler for C++ binaries."""
...
@dataclass
class TimingMarkers:
    """Timing markers extracted from binary stderr output."""
...
class ProfilingRunner:
    """Orchestrates a profiler to analyze a C++ test binary."""
    ...
    def run(self, test_binary: Path) -> ProfilingResult:
        """
        Run the profiler on the test binary.

        Args:
            test_binary: Path to the C++ test binary
        ...
        cmd = self.profiler.build_command([str(test_binary)])
        ...
        if result.returncode != 0:
            print(
                f"WARNING: Process exited with code {result.returncode}",
                file=sys.stderr,
            )
    ...
    def _extract_timing_markers(self, stderr: str) -> TimingMarkers:
        ...
        if not (start_match and end_match and total_match):
            # Try reading from profiler log if markers are there
            log_file = getattr(self.profiler, "log_file", None)
            if log_file and log_file.exists():
                content = log_file.read_text()
                start_match = start_match or re.search(r"===START=(\d+)===", content)
